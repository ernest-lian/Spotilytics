from backend.src.analytics_model.popular_track_dao.popular_track_dao import PopularTrackDao
from backend.src.analytics_model.energetic_track_dao.energetic_track_dao import EnergeticTrackDao
from backend.src.analytics_model.fastest_track_dao.fastest_track_dao import FastestTrackDao
from backend.src.analytics_model.longest_track_dao.longest_track_dao import LongestTrackDao
...
class AnalyticsModel:
    ...
    def fetch_analytics(sp, top_tracks):
        ...
        energetic_track['average'] = round((average_energy / 20) * 100, 2)
    ...
    def store_analytics(user_id, sp, top_tracks):
        response = {}
        analytics = AnalyticsModel.fetch_analytics(sp, top_tracks)

        try:
            response['popular_track'] = PopularTrackDao.create_connection(user_id, AnalyticsModel.fetch_popular(sp))
            response['fastest_track'] = FastestTrackDao.create_connection(user_id, analytics['fastest_track'])
            response['energetic_track'] = EnergeticTrackDao.create_connection(user_id, analytics['energetic_track'])
            response['longest_track'] = LongestTrackDao.create_connection(user_id, analytics['longest_track'])
            return response
